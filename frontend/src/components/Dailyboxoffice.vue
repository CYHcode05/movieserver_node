...
export default {
  ...
  methods: {
    selectMovie(movie) {
      ...
      console.log(this.$store.state.selectedMovieid);
    ...
    async fetchDailyBoxOffice() {
      try {
        ...
        const response = await fetch(`http://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json?key=f5eef3421c602c6cb7ea224104795888&targetDt=${year}${month}${date}`);
        const data = await response.json();
        const movies = data.boxOfficeResult.dailyBoxOfficeList.slice(0, 10);
        ...
        this.movies = await Promise.all(movies.map(async (movie) => {
          ...
          return {
            ...
            typeNm: movie.typeNm,
            openDt: movie.openDt,
            tmdbid: tmdbMovie ? tmdbMovie.id : null,
          };
        }));
        this.nextMovies = [...this.movies];
        this.visibleMovies = this.nextMovies.slice(0, 4);
      } catch (error) {
        console.error('Failed to fetch daily box office:', error);
      }
    },
  },
  components: {
...
.movie-block {
  ...
  flex: 0 0 auto;
  width: 20%;
  /* Adjust this value to change the size of the movie block */
}
