const SocekIO = require('socket.io');
const Evaluation = require('./evaluation.model.js');

module.exports = (server) => {
    const io = SocekIO(server, {
        path: '/socket.io',
        cors:{
    ...
    io.on("connection", (socket) => {       //io.sockets.on과 동일
        ...
        socket.interval = setInterval(() => { // send 대신 emit으로 메세지를 보냄
            sendDatabaseInfoToClient(socket) 
        }, 5000);
...
async function sendDatabaseInfoToClient(socket) {
    try {
        const evaluations = await Evaluation.find();
        socket.emit("databaseInfo", evaluations);
    } catch (error) {
        console.error("Error sending database info to client:", error);
    }
}